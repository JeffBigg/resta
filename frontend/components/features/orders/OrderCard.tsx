...
// Configuración de estados con colores semánticos o utilitarios que funcionan en ambos modos
// Usamos opacidades (/10 o /20) para que el fondo se adapte automáticamente.
...
export default function OrderCard({ pedido, repartidores, onUpdate }: Props) {
  ...
  return (
    <article className={`
        group relative w-full h-full flex flex-col bg-card border border-border rounded-3xl
        shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 ease-out overflow-hidden
    ...
    `}>
      ...
      {!isCancelled && !isDelivered && (
         <button 
           ...
           disabled={loading}
           className="absolute top-3 right-3 z-20 p-2 rounded-full bg-card/80 text-muted-foreground hover:text-destructive hover:bg-destructive/10 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 scale-90 group-hover:scale-100 shadow-sm border border-border"
      ...
      {/* 2. HEADER */}
      <div className={`px-5 py-4 flex justify-between items-center border-b border-border bg-muted/20`}>
        {/* Badge de Estado */}
      ...
      <div className="p-5 flex-1 flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-card-foreground leading-tight truncate pr-2" title={pedido.cliente_nombre}>
        ...
        <div className="flex gap-3 items-start">
             ...
             <p className="text-xs text-muted-foreground font-medium leading-relaxed line-clamp-2 pt-0.5">
        ...
        <div className="bg-muted/30 rounded-xl p-3 border border-border">
            <ul className="space-y-2">
                ...
                <li key={i} className="flex items-start justify-between text-xs text-foreground">
                    {typeof item === 'string' ? (
                        <span>{item}</span>
                    ) : (
                        <>
                            <span className="truncate font-medium">{item.nombre}</span>
                            <span className="shrink-0 font-bold bg-background px-1.5 py-0.5 rounded border border-border text-[10px] text-muted-foreground">
                                x{item.cantidad}
                            </span>
                        </>
                    )}
                ...
                {pedido.detalle_pedido.items.length > 3 && (
                    <li className="text-[10px] text-center font-bold text-muted-foreground pt-1">
      ...
      <div className="p-4 bg-muted/20 border-t border-border mt-auto">
        ...
        {['Cocina', 'Listo_para_recoger'].includes(pedido.status_entrega) && (
          <div className="flex gap-2 animate-in fade-in duration-300">
            <div className="relative flex-1 group/input">
              <select 
                className="w-full appearance-none bg-background hover:bg-accent border border-input text-foreground text-xs font-semibold rounded-xl px-3 py-3 pr-8 focus:outline-none focus:ring-2 focus:ring-ring focus:border-input transition-all cursor-pointer"
                value={selectedRider}
              ...
              <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none text-muted-foreground group-hover/input:text-primary transition-colors">
            ...
            <button
              ...
              className="bg-primary hover:bg-primary/90 text-primary-foreground p-3 rounded-xl shadow-lg shadow-primary/20 hover:shadow-primary/40 disabled:opacity-50 disabled:shadow-none transition-all active:scale-95 flex items-center justify-center aspect-square"
            >
              {loading ? (
        ...
        {pedido.status_entrega === 'En_ruta' && (
           <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="flex items-center gap-2 px-3 py-2 bg-indigo-500/10 rounded-lg border border-indigo-500/20">
                 <div className="w-6 h-6 rounded-full bg-background flex items-center justify-center text-[10px] font-bold text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800">
              ...
              <button
                 ...
                 disabled={loading}
                 className="w-full bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold py-3 px-4 rounded-xl shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
              >
        ...
        {isDelivered && (
           <div className="flex flex-col items-center justify-center py-1 animate-in zoom-in duration-300">
              ...
              <div className="px-4 py-1.5 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 font-mono text-sm font-bold rounded-lg border border-emerald-500/20">
                  {deliveredTime}
              </div>
           </div>
        )}
        ...
        {isCancelled && (
           <div className="text-center py-2">
              <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Cancelado</span>
           </div>
        )}
