...
export default function EmployeesView() {
  ...
  const handleResult = (success: boolean, type: string, isEdit: boolean) => {
    if (success) {
      toast.success(
        `${type} ${isEdit ? "actualizado" : "registrado"} correctamente`
      );
      handleCancel();
      loadData(true);
    } else {
      ...
      {isFormOpen && (
        <div className="bg-card border border-border rounded-xl p-6 shadow-lg animate-in slide-in-from-top-2">
          <h3 className="font-bold text-foreground mb-4 pb-2 border-b border-border flex justify-between items-center">
            <span>
              {editingId ? "✏️ Editando" : "➕ Registrando"}{" "}
              {activeTab === "staff" ? "Personal" : "Motorizado"}
            </span>
            {editingId && (
          ...
          >
            <div>
              ...
              <input
                type="text"
                required
                className="w-full bg-secondary/30 border border-input rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/50 text-foreground"
            ...
            <div>
              ...
              <input
                type="tel"
                required
                className="w-full bg-secondary/30 border border-input rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/50 text-foreground font-mono"
            ...
            <div>
              ...
              <input
                ...
                required
                maxLength={4}
                className="w-full bg-secondary/30 border border-input rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/50 text-foreground font-mono tracking-widest"
                value={formData.pin_code}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    pin_code: e.target.value.slice(0, 4),
                  })
                }
                placeholder="0000"
              />
            ...
            {activeTab === "staff" && (
              <div className="md:col-span-4">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, rol_operativo: e.target.value })
                  }
                >
            ...
            <button
              type="submit"
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2 rounded-lg text-sm shadow transition-colors md:col-span-4 h-10 mt-2"
            >
